...
plugins=(
		git fzf pip sudo zsh-interactive-cd
		docker docker-compose z zsh-syntax-highlighting
		jenv
		direnv
		nvm
	)
...
venv_prompt_info() {
  [[ -n "$VIRTUAL_ENV" ]] && echo "%K{green}${VIRTUAL_ENV:t}%k "
}

git_prompt_truncated() {
  local branch
  branch=$(git_current_branch 2>/dev/null)
  [[ -z "$branch" ]] && return

  # Truncate to 16 chars max
  [[ ${#branch} -gt 16 ]] && branch="${branch:0:16}"
  ...
  echo "%{$fg_bold[blue]%}git:(%{$fg[red]%}${branch}%{$fg_bold[blue]%}) ${icon}%{$reset_color%} "
}
